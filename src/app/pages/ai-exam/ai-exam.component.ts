...
export class AiExamComponent implements OnInit {
    ...
    initializeExam() {
        ...
        this.aiExamService.generateQuestions(this.contentId).subscribe({
            ...
            error: (error) => {
                ...
                this.showError(
                    'Failed to connect to AI system. Please check your connection and try again.'
                );
                this.isThinking = false;
            },
        });
    ...
    submitAnswer() {
        if (!this.currentAnswer.trim()) {
            this.showError(
                'Please provide an answer before submitting to the AI.'
            );
            return;
        }
        ...
        this.aiExamService
            ...
            .subscribe({
                ...
                error: (error) => {
                    ...
                    this.showError(
                        'Failed to get AI feedback. Please check your connection and try again.'
                    );
                },
                complete: () => {
    ...
    getFinalEvaluation() {
        ...
        debugger;
        this.aiExamService
            .finalEvaluation(this.contentId, this.studentAnswers)
            .subscribe({
                next: (res) => {
                    if (res.isSuccess) {
                        // ✅ Passed exam
                        ...
                        this.finalEvaluation = res.data!;
                        this.showError(
                            res.message || 'لم تنجح في الامتحان. حاول مرة أخرى.'
                        );
                    }
                    this.isLoading = false;
                },
                error: (error) => {
                    ...
                    this.showError(
                        'Failed to get AI final assessment. Please check your connection and try again.'
                    );
                    this.isLoading = false;
                },
                complete: () => {
    ...
    goToNextContent() {
        if (this.nextContentId > 0) {
            this.router.navigate(['/lecture-viewer', this.programId], {
                queryParams: { contentId: this.nextContentId },
            });
        }
    }
